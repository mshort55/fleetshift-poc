...
-- Backfill: each existing deployment -> fulfillment + strategy v1 records.
...
SELECT
    id, manifest_strategy, 1,
    placement_strategy, 1,
    rollout_strategy, CASE WHEN rollout_strategy IS NOT NULL THEN 1 ELSE 0 END,
    resolved_targets, state, status_reason, auth, provenance,
    generation, observed_generation, active_workflow_gen,
    created_at, updated_at
FROM deployments;
...
SELECT id, 1, rollout_strategy, created_at FROM deployments WHERE rollout_strategy IS NOT NULL;

-- Update delivery_records to reference fulfillment_id (same value as old deployment_id for backfill).
ALTER TABLE delivery_records RENAME COLUMN deployment_id TO fulfillment_id;

-- Recreate deployments as thin table.
